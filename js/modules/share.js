...
export function buildShareBar(title, small) {
    ...
    return `<div class="${cls}" onclick="event.stopPropagation()">
        <span>Compartir</span>
        <a href="https://www.facebook.com/sharer/sharer.php?u=${url}" target="_blank" rel="noopener" class="fb" aria-label="Compartir en Facebook">${ICONS.fb}</a>
        <a href="https://wa.me/?text=${text}%20${url}" target="_blank" rel="noopener" class="wa" aria-label="Compartir en WhatsApp">${ICONS.wa}</a>
    </div>`;
}
